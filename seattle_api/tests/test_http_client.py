...
from seattle_api.http_client import SeattleHTTPClient, HTTPClientError
...
class TestSeattleHTTPClient:
    ...
    async def test_fetch_incident_html_success(self, http_client, mock_response):
        ...
        test_html = "<html><body><table><tr><td>incident data</td></tr></table></body></html>"
    ...
    async def test_fetch_incident_html_invalid_html(self, http_client, mock_response):
        ...
        with patch.object(httpx.AsyncClient, 'get') as mock_get:
            ...
            with pytest.raises(HTTPClientError, match="does not appear to be valid HTML"):
                await http_client.fetch_incident_html()
    
    @pytest.mark.asyncio
    async def test_fetch_incident_html_http_error_4xx(self, http_client):
        """Test handling of 4xx HTTP errors (no retry)."""
        error_response = Response(404, request=Request("GET", "https://test.example.com"))
        http_error = httpx.HTTPStatusError("Not Found", request=error_response.request, response=error_response)
    ...
    @pytest.mark.asyncio
    async def test_fetch_incident_html_http_error_5xx_with_retry(self, http_client, mock_response):
        """Test handling of 5xx HTTP errors with retry."""
        error_response = Response(500, request=Request("GET", "https://test.example.com"))
        http_error = httpx.HTTPStatusError("Server Error", request=error_response.request, response=error_response)
    ...
    def test_is_valid_html_response(self, http_client):
        ...
        assert http_client._is_valid_html_response("<table><tr><td>data</td></tr></table>")
    ...
    async def test_auto_start_client(self, http_client, mock_response):
        ...
        with patch.object(httpx.AsyncClient, 'get') as mock_get:
            mock_get.return_value = mock_response(200, "<html><table>test</table></html>")
