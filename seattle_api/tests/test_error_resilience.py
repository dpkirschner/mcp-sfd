...
from seattle_api.circuit_breaker import CircuitState
...
from seattle_api.poller import IncidentPoller
...
def mock_http_client():
    ...
    client.fetch_incident_html = AsyncMock(
        return_value="<html><body>Mock HTML response</body></html>"
    )
    return client
...
class TestCircuitBreakerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_http_circuit_breaker_opens_on_failures(
        self, poller, mock_http_client
    ):
        ...
        mock_http_client.fetch_incident_html.side_effect = httpx.TimeoutException(
            "Connection timeout"
        )
    ...
    @pytest.mark.asyncio
    async def test_parsing_circuit_breaker_opens_on_failures(
        self, poller, mock_http_client
    ):
    ...
    async def test_circuit_breaker_recovery(self, poller, mock_http_client):
        ...
        mock_http_client.fetch_incident_html.side_effect = httpx.ConnectError(
            "Connection failed"
        )
        result1 = await poller.poll_once()
        assert result1 is False
...
class TestGracefulDegradation:
    ...
    @pytest.mark.asyncio
    async def test_serve_from_cache_during_http_failure(
        self, poller, mock_http_client, mock_cache, sample_incident
    ):
    ...
    @pytest.mark.asyncio
    async def test_serve_from_cache_during_parsing_failure(
        self, poller, mock_http_client, mock_cache, sample_incident
    ):
    ...
    @pytest.mark.asyncio
    async def test_exit_degraded_mode_on_success(
        self, poller, mock_http_client, mock_cache, sample_incident
    ):
    ...
    @pytest.mark.asyncio
    async def test_degraded_mode_without_cache(
        self, poller, mock_http_client, mock_cache
    ):
...
class TestErrorLogging:
    ...
    async def test_circuit_breaker_logging(self, poller, mock_http_client, caplog):
        ...
        assert any(
            "Circuit breaker" in record.message and "opened" in record.message
            for record in info_logs
        )

    @pytest.mark.asyncio
    async def test_degraded_mode_logging(
        self, poller, mock_http_client, mock_cache, sample_incident, caplog
    ):
        ...
        assert any(
            "Entering degraded mode" in record.message for record in warning_logs
        )
...
class TestErrorRecoveryScenarios:
    ...
    @pytest.mark.asyncio
    async def test_cache_update_failure_handling(
        self, poller, mock_http_client, mock_cache
    ):
