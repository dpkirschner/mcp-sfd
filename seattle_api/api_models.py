...
class IncidentResponse(APIResponse):
    ...
    data: Optional[Incident] = Field(None, description="Incident data")
...
class HealthResponse(BaseModel):
    ...
    config: Dict[str, Any] = Field(default_factory=dict, description="Configuration details")
    poller_status: Optional[Dict[str, Any]] = Field(None, description="Poller health status")


class IncidentSearchParams(BaseModel):
    """Query parameters for incident search."""

    limit: Optional[int] = Field(100, ge=1, le=1000, description="Maximum number of incidents to return")
    offset: Optional[int] = Field(0, ge=0, description="Number of incidents to skip")
    status: Optional[IncidentStatus] = Field(None, description="Filter by incident status")
    incident_type: Optional[str] = Field(None, description="Filter by incident type (partial match)")
    address: Optional[str] = Field(None, description="Filter by address (partial match)")
